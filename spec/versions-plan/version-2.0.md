...
- [x] 调研将元感知网站H5页面集成到公众号菜单的流程（见第六章）
- [x] 调研微信用户身份同步方案（见第六章）：
...
| 阶段3 | 服务配置（systemd/Nginx反向代理/SSL证书）       | ✅ 完成 |
| 阶段4 | 前端集成验证（微信OAuth登录）                   | ✅ 完成 |

**当前状态**：后端已部署完成，微信OAuth登录已验证通过 ✅

**线上地址**：

- 后端 API：`https://api.metamindpt.com`
- 前端 H5：`https://www.metamindpt.com/app/`
...
> **背景说明**：
>
> - **Core UI** (ysx-core-ui)：元感知前端，部署在阿里云 www.metamindpt.com
> - **明镜前端**：仍在 Vercel，暂不处理
...
| 后端用户模块设计   | ✅ 已完成（见第七章）            |
| **后端阿里云部署** | ✅ **已完成**（见第八章）        |
| 后端用户模块开发   | ✅ 已完成（微信OAuth登录已验证） |
| 前端微信登录集成   | ✅ 已完成（见第十二章开发日志）  |
...
**元感知不是静心疗愈产品，而是文明级、镜面级、碑感级的入口平台。**
...
> 本章节记录当日开发进度、问题修复和重要决策

### 1. 微信登录流程问题修复

#### 1.1 问题：首页出现两次

**现象**：在微信中打开链接时，首页会先显示一次，然后微信授权完成后又显示一次。

**原因**：在初始化完成前就渲染了页面内容。

**解决方案**：在 `App.tsx` 中添加 `loading` 状态控制，初始化完成前返回 `null`。
...
**注意事项**：`if (loading) return null;` 必须放在所有 React Hooks **之后**，否则会违反 Hooks 规则导致页面空白。

#### 1.2 问题：用户信息交替丢失

**现象**：有时能获取到用户信息，有时获取不到，交替出现。

**原因**：token 过期后没有重新触发微信授权。

**解决方案**：在 `useUserStore.ts` 中，当 token 无效时检测是否在微信浏览器，如果是则重新跳转授权。
...
// useUserStore.ts
  ...
  // 非微信浏览器，正常显示未登录状态
  setState({ isLoggedIn: false, userInfo: null, loading: false, error: null });
}
```

#### 1.3 防止重复初始化

使用 `useRef` 确保 `initialize` 函数只执行一次：
...
在 Safari 浏览器中，页面橡皮筋滚动（overscroll bounce）时会露出棕色背景，而 Chrome 和微信中没有这个问题。

#### 原因分析

Safari 的橡皮筋滚动效果会显示 `<meta name="theme-color">` 指定的颜色。原来的 `theme-color` 是 `#8B7355`（棕色），这是旧版 UI 的主题色。

#### 解决方案

将 `index.html` 中的 `theme-color` 改为冷白色 `#F5F5F7`：
...
| 浏览器       | Overscroll 行为                    |
| ------------ | ---------------------------------- |
| Safari       | 显示 `theme-color` 的颜色          |
| Chrome       | 不显示橡皮筋效果，或显示默认白色   |
| 微信 WebView | 与 Chrome 类似，基于 Chromium 内核 |

---

### 3. 后端用户模块更新

#### 3.1 添加 login_count 字段

在 `auth_routes.py` 中添加登录计数字段：
...
user_data = {
    ...
    "last_login_at": aware_utc_now,
    "login_count": 1,  # 新用户初始为1
}

# 老用户
update_data = {
    "last_login_at": aware_utc_now,
    "$inc": {"login_count": 1}  # 每次登录+1
}
```
...
将 `index.html` 中的标题从 "元感知 - 身心成长平台" 改为 "元感知"：
...
将链接发给其他人时，如果对方没有关注测试号，会提示"微信登录失败，请关注此测试号"。

#### 原因

微信测试号的固有限制：只有关注了测试号的用户才能授权登录。

#### 解决方案

| 方案                   | 适用场景     | 说明                                   |
| ---------------------- | ------------ | -------------------------------------- |
| **让测试者关注测试号** | 当前测试阶段 | 把测试号二维码发给测试者，先关注再测试 |
| 添加"游客模式"         | 可选扩展     | 允许未登录用户浏览，需要时才登录       |
| 申请正式服务号         | 上线前必须   | 正式服务号支持所有微信用户授权         |
...
```bash
# 验证 theme-color 是否更新
curl -s "https://www.metamindpt.com/app/index.html" | grep -q "theme-color.*F5F5F7" && echo "✅ 线上已更新" || echo "❌ 线上未更新"
```
...
# 上传到服务器
scp -r /Users/chrisdu/ysx-core-ui/deploy/* ubuntu@<服务器IP>:/var/www/html/
```
